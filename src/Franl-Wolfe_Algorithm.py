...
def train_frank_wolfe(type_num, tr_offerset_list, tr_sell_list, tr_mask_list, opt='adam'):
    ...
    with torch.no_grad():
        ...
        for proportion, fw_func in zip(problem.proportion, problem.taste_list):
            ...
            f += proportion * fw
        f_log = torch.log(f)
        evaluation_loss = nn.NLLLoss()(f_log, torch.tensor(val_sell, dtype=torch.int64, device=device).argmax(dim=1))  # calculate test loss
    ...
    for m in range(n):
        ...
        with torch.no_grad():
            ...
            print('Evaluation loss: ', evaluation_loss.item())  
    ...
    return problem, NLL_LOSS_LIST, EVALUATION_LOSS_LIST, TEST_LOSS_LIST #return
